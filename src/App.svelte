<script>
  ...
  function updateImageObject(objectId, payload) {
    ...
    let tempAllObjects = [];
    for (var i = 0; i < allObjects.length; i++) tempAllObjects[i] = [];
    ...
    for (var i = 0; i < allObjects.length; i++) {
      ...
      for (var j = 0; j < objects.length; j++) {
        ...
        if (object.y < 0) {
          ...
          object.width = object.originalWidth;
  ...
  function onMeasure(scale, width, height, i) {
    pagesScale[i] = scale;
    canvasWidth = width;
    canvasHeight = height;
  }
      ...
      {#each pages as page, pIndex (page)}
        ...
        >
          ...
          >
            <PDFPage
              on:measure={(e) =>
                onMeasure(e.detail.scale, e.detail.canvasWidth, e.detail.canvasHeight, pIndex)}
              {page}
            />
            <div
